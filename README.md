...
- 가우시안·salt-pepper·burst 세 종류 노이즈로 `−30 ~ −10dB` 구간을 조건화하고, 복원·분류 2-head 구조로 학습. 입력은 복소 IQ, 복원 head의 출력은 STFT 스펙트로그램

**결과와 종료 판단**
심볼 분류 정확도를 SNR 구간별로 비교했습니다.

| SNR | Baseline | BAM | 차이 |
|---|---|---|---|
| −30 dB | 3.5% | 7.1% | +3.6%p |
| −25 dB | 18.7% | 27.8% | +9.1%p |
| −20 dB | 80.4% | 81.4% | +1.0%p |
| −15 dB | 100.0% | 99.9% | −0.1%p |
| −10 dB | 100.0% | 100.0% | 0.0%p |
| −5 dB | 100.0% | 100.0% | 0.0%p |

개선은 baseline이 무너지는 −30 ~ −25dB 두 구간에만 나타났고, 그 구간의 절대 정확도는 여전히 실용 수준과 멀리 있습니다. baseline이 이미 동작하는 −15dB 이상에서는 차이가 없습니다. −20 ~ −15dB의 1%p 안킎 차이는 단일 실행 기준이므로 개선으로 주장하지 않습니다.
